...
class ElasticsearchTaskHandler(FileTaskHandler, LoggingMixin):
    ...
    def es_read(self, log_id, offset):
        ...
        execution_date = '_'.join(log_components[2:5]).replace(":", "_").replace("-", "_")
        # try_number = str(int(log_components[5]) - 1)
        try_number = str(int(log_components[5]))

        log_id = f"{dag_id}_{task_id}_{execution_date}_{try_number}"
